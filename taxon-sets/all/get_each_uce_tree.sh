...
uce_trimmed_dict="/home/projects/ku_00039/people/zelili/GAGA/taxon-sets/all/mafft-nexus-internal-trimmed-gblocks-fasta"
cd $uce_trimmed_dict

#for uce in $(ls aln_oneline_uce-* | sed -e 's/aln_oneline_//g' | sed -e 's/.fasta//g'); do
for uce in $(ls *.fasta | awk -F '.' '{print $1}'); do
	echo -e "#!/bin/bash\n#PBS -N ${uce}\n#PBS -l walltime=240:00:00\n#PBS -e each_uce_tree_err.log\n#PBS -o each_uce_tree_out.log\nmodule load tools\nmodule load ngs\nmodule load anaconda3/4.4.0\nmodule load perl\nmodule load trimal/1.4.1\nmodule load muscle/3.8.425\nmodule load iqtree/2.1.2\ncd ${uce_trimmed_dict}\niqtree2 -s ${uce_trimmed_dict}/${uce}.fasta --prefix tree_${uce} -B 1000 -m MFP -T 1 --safe" > $uce_trimmed_dict/each_uce_tree.sh
	#echo -e "#!/bin/bash\n#PBS -N ${uce}\n#PBS -l walltime=240:00:00\n#PBS -e each_uce_tree_err.log\n#PBS -o each_uce_tree_out.log\nmodule load tools\nmodule load ngs\nmodule load anaconda3/4.4.0\nmodule load perl\nmodule load trimal/1.4.1\nmodule load muscle/3.8.425\nmodule load iqtree/2.1.2\ncd ${uce_dict}\niqtree2 -s ${uce_dict}/aln_oneline_${uce}.fasta --prefix tree_${uce} -B 1000 -m MFP -T 1 --safe" > $uce_dict/each_uce_tree.sh
	qsub $uce_trimmed_dict/each_uce_tree.sh
